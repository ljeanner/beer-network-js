...
async function payOut(shipmentRecu) {  // eslint-disable-line no-unused-vars
    ...
    const contract = shipmentRecu.shipment.contract;
    const shipment = shipmentRecu.shipment;
    ...
    // si l'Shipment n'est pa recu en temps le solde est egale 0 
...
/*
async function exchange(credite, crediteur, ShipmentRecu) {
    
    const payOut = payOut(ShipmentRecu)
    console.log("Payout: " + payOut);
	Contract.credite.accountBalance += payOut;
	Contract.crediteur.accountBalance -= payOut;

	console.log(Contract.credite.$identifier + "\new balance: " + Contract.credite.accountBalance);
	console.log(Contract.crediteur.$identifer + "\new balance: " + Contract.crediteur.accountBalance);

	// MAJ du solde du credite
	const registreCredite = await getParticipantRegistry("org.acme.beer." + Contract.credite) //.$identifier);
	await registreCredite.update(Contract.credite);

	// MAJ du solde du crediteur
	const registreCrediteur = await getParticipantRegistry("org.acme.beer." + Contract.crediteur) //.$identifier);
	await registreCrediteur.update(Contract.crediteur);
